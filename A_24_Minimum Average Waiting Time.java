#include <bits/stdc++.h>

using namespace std;

string ltrim(const string &);
string rtrim(const string &);
vector<string> split(const string &);

/*
 * Complete the 'minimumAverage' function below.
 *
 * The function is expected to return an INTEGER.
 * The function accepts 2D_INTEGER_ARRAY customers as parameter.
 */

long long minimumAverage(vector<vector<int>> customers) {
    // Sort customers by arrival time
    sort(customers.begin(), customers.end(), [](const vector<int>& a, const vector<int>& b) {
        return a[0] < b[0];
    });

    // Min-heap storing pair<cooking_time, arrival_time>
    priority_queue<pair<long long, long long>, 
                   vector<pair<long long, long long>>, 
                   greater<pair<long long, long long>>> min_heap;

    long long total_wait_time = 0;
    long long current_time = 0;
    int index = 0;
    int n = customers.size();

    while (index < n || !min_heap.empty()) {
        // Add all customers who have arrived by current_time
        while (index < n && customers[index][0] <= current_time) {
            min_heap.push({customers[index][1], customers[index][0]});
            index++;
        }

        // If no customers are waiting, jump current_time to the next arrival
        if (min_heap.empty()) {
            current_time = customers[index][0];
            continue;
        }

        // Serve the customer with the shortest cooking time (Shortest Job First)
        auto top = min_heap.top();
        min_heap.pop();

        long long cook_time = top.first;
        long long arrival_time = top.second;

        current_time += cook_time;
        total_wait_time += (current_time - arrival_time);
    }

    return total_wait_time / n;
}

int main()
{
    ofstream fout(getenv("OUTPUT_PATH"));

    string n_temp;
    getline(cin, n_temp);

    int n = stoi(ltrim(rtrim(n_temp)));

    vector<vector<int>> customers(n);

    for (int i = 0; i < n; i++) {
        customers[i].resize(2);

        string customers_row_temp_temp;
        getline(cin, customers_row_temp_temp);

        vector<string> customers_row_temp = split(rtrim(customers_row_temp_temp));

        for (int j = 0; j < 2; j++) {
            int customers_row_item = stoi(customers_row_temp[j]);

            customers[i][j] = customers_row_item;
        }
    }

    int result = minimumAverage(customers);

    fout << result << "\n";

    fout.close();

    return 0;
}

string ltrim(const string &str) {
    string s(str);

    s.erase(
        s.begin(),
        find_if(s.begin(), s.end(), not1(ptr_fun<int, int>(isspace)))
    );

    return s;
}

string rtrim(const string &str) {
    string s(str);

    s.erase(
        find_if(s.rbegin(), s.rend(), not1(ptr_fun<int, int>(isspace))).base(),
        s.end()
    );

    return s;
}

vector<string> split(const string &str) {
    vector<string> tokens;

    string::size_type start = 0;
    string::size_type end = 0;

    while ((end = str.find(" ", start)) != string::npos) {
        tokens.push_back(str.substr(start, end - start));

        start = end + 1;
    }

    tokens.push_back(str.substr(start));

    return tokens;
}
