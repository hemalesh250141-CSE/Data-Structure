...
int cookies(int k, vector<int> A) {
    // Min-heap to store cookie sweetness values
    priority_queue<int, vector<int>, greater<int>> min_heap(A.begin(), A.end());
    
    int operations = 0;

    // Continue combining while the least sweet cookie is less than k
    while (min_heap.size() >= 2 && min_heap.top() < k) {
        ...
        min_heap.push(new_cookie);
        
        operations++;
    }

    // Check if all cookies meet the threshold k
